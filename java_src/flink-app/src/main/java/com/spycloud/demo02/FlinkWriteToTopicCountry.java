...
public class FlinkWriteToTopicCountry {
    ...
    private static List<String> generateFakePersons(int count) {
        ...
        List<Country> knownCountryList = Arrays.asList(
            new Country("US", "United States", 331_002_651L),
            new Country("DE", "Germany", 83_783_942L),
            new Country("FR", "France", 67_084_000L)
        );
        countryList.addAll(knownCountryList);

        for (int i=0;i<count;i++){
            Country country = new Country(faker.country().countryCode2(),  faker.country().name(), faker.number().randomNumber(6, true));
